	...
	public class BulletHitFX : MonoBehaviour
	{
		...
		public static void SpawnDecal(RaycastHit hit, Part hitPart, float caliber, float penetrationfactor)
		{
			...
			if (decalFront != null && hitPart != null)
			{
				...
				if (CanFuelDrain(hitPart))
				{
					AttachLeak(hit, hitPart, caliber);
				}
			}
		...
		private static bool CanFuelDrain(Part hitPart)
		{
			if (BDArmorySettings.SELFSEALING_TANKS || BDArmorySettings.PAINTBALL || !hitPart.HasFuel())
				return false;

			if (!hitPart.Resources.Contains("LiquidFuel"))
				return false;
			/*
		...
		{
			var modelUrl = "BDArmory/FX/FuelLeakFX/model";
			...
			//DecalEmitterScript.lifeTime = 20;

			var leak = hitPart.FindModuleImplementing<ModuleDrainFuel>();
			if (leak != null)
			{
				leak.drainRate += ((caliber / 100));
			}
			else
			{
				hitPart.AddModule("ModuleDrainFuel");
				leak.drainRate = ((caliber / 100));
				leak.drainDuration = 20;
			}
